...
```bash
# 1. 解析邮件，更新 shipping 缓存，将新订单写入暂存区
python main.py
...
> `main.py` 可随时重复运行 — 已处理的邮件会被 `seen_mails.json` 跳过，不会重复解析。
>
> `sync_orders.py` 也是幂等的 — 每次运行会先删除多维表中今天的旧数据再全量写入，可放心重复执行。
...
- 按业务日期分类：今天 / 未来 / 异常
- 对今日订单按**省份 → 城市 → 收货地址 → 订单号**排序
- 输出排序后的 `output/orders_YYYY-MM-DD.json`（供发货拼单参考）
- 先删除多维表中今天的旧数据，再全量写入新数据（幂等）
...
|---|---|---|
| 1 | 规范化地址与多维表记录**精确匹配**，匹配到的收货单位全部相同 | `一致` |
| 2 | 精确匹配命中**多个不同收货单位**，通过评分选最优 | `多关系对应` |
| 3 | 精确匹配无结果，用**动态规划可分词匹配**（地址可被拆分为均在文本池中出现的子串）| `模糊匹配` |
| 4 | 所有匹配失败，对全库按**字符重合度评分**取最高分兜底（永不留空）| `模糊匹配` |
...
| `多关系对应` | 地址精确匹配到多维表中多个**不同的**收货单位，通过评分选出最优（需人工复核） |
...
| 地址状态 | `address_exact_match` | 一致 / 多关系对应 / 模糊匹配 |
| 收货单位 | `receiver` | 多维表收货单位映射匹配结果 |
| 公司名 | `company_name` | PDF 客户字段 `~` 后提取的客户公司名 |
